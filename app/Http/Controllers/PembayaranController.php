...
public function riwayat(Request $request)
{
    ...
    if ($salesId = $request->sales_id) {
        $query->where(function ($q) use ($salesId) {
            // pembayaran langsung oleh sales di field id_sales
            $q->where('id_sales', $salesId)
              // atau pembayaran admin tapi langganan milik sales tertentu
    ...
    // Jika request AJAX (realtime filter/search)
if ($request->ajax()) {
...
public function hapusItem(Request $request, $id)
{
    ...
    try {
        ...
        // 1. Kembalikan status tagihan jadi "belum lunas"
        if ($tagihan) {
            $tagihan->update([
        ...
        // 2. Kurangi nominal di tabel pembayaran
        if ($pembayaran) {
            $pembayaran->update([
        ...
        // 4. Kalau pembayaran sudah tidak punya item lagi → boleh sekalian dihapus
...
public function hapusItemBulk(Request $request)
{
    ...
    try {
        ...
        // recalculasi nominal per pembayaran
